...
def init_vel(N,sigma):
    ...
    initVel = max((v_x**2 + v_y**2 + v_z**2)**(1/2))
    for n in range(N):
        velocity = [v_x[n],v_y[n],v_z[n]]
        velList.append(velocity)
        vel = np.array(velList)
        initVelList.append(initVel)
    return vel,initVelList
...
def getAcc(pos, mass, G, softening):
    ...
    inv_x3 = (dx ** 2 + softening ** 2)
    # print(inv_x3)
    # print("------")
    inv_x3[inv_x3 > 0] = inv_x3[inv_x3 > 0] ** (-1.5)
    print(inv_x3)
    print("--------")

    inv_y3 = (dy ** 2 + softening ** 2)
    inv_y3[inv_y3 > 0] = inv_y3[inv_y3 > 0] ** (-1.5)

    inv_z3 = (dz ** 2 + softening ** 2)
    inv_z3[inv_z3 > 0] = inv_z3[inv_z3 > 0] ** (-1.5)

    ax = G * (dx * inv_r3) @ mass
    ay = G * (dy * inv_y3) @ mass
    az = G * (dz * inv_z3) @ mass

    total_a = (ax ** 2 + ay ** 2 + ax ** 2) ** (1 / 2)
...
def velMax_and_t(tList,maxVelList):
    plt.scatter(tList, maxVelList, marker=".")
    plt.show()

def dt_and_t(tList,dtList):
    plt.scatter(tList,dtList,marker=".")
    plt.show()
...
def main():
    ...
    #初期速度分散
    sigma = (2*r_v*abs(PE)/(3*M))

    # print(sigma)
    #
    # return 0
    vel,initVelList = init_vel(N,sigma) #粒子の速度の生成
    maxVel = max(initVelList)
    ...
    acc,totalAcc = getAcc(pos, mass, G, softening) #初期位置での相互重力作用の計算
    ...
    while tEnd > t_step:
        ...
        pos += vel * dt
        #　粒子移動後の加速度の取得
        acc,totalAcc = getAcc(pos, mass, G, softening)
        #dt後のvelの計算
        vel += acc*dt/2
